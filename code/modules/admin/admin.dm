...
/datum/admins/proc/saveallworld()
	set category = "Server"
	set name = "Save World"
	set desc="Saves World Data"
	if (!usr.client.holder)
		return
	var/confirm = alert("Backup the game world?", "Save", "Yes", "Cancel")
	if(confirm == "Cancel")
		return
	if(confirm == "Yes")
		world << "<span class='danger'>Backing up the world!</span> <span class='notice'>Initiated by [usr.client.holder.fakekey ? "Admin" : usr.key]!</span>"
		log_admin("[key_name(usr)] initiated a reboot.")

		feedback_set_details("end_error","admin backup - by [usr.key] [usr.client.holder.fakekey ? "(stealth)" : ""]")
